...
try
    ...
    println("      Julia features.")
    println()
    println("Common issues:")
    println("  - Dynamic dispatch and runtime type inference")
    println("  - Certain standard library functions")
    println("  - Missing Rust libraries (run: julia deps/build.jl)")
    println("  - Missing clang compiler (required for StaticCompiler)")
    println()
    println("Solutions:")
    println("  1. Ensure Rust libraries are built: julia deps/build.jl")
    println("  2. Install clang: sudo apt-get install clang (Linux)")
    println("  3. Verify setup: julia --project=. scripts/verify_static_compilation_setup.jl")
    println()
    rethrow(e)
end
...
println()
println("Run it with:")
println("  $(joinpath(OUTPUT_DIR, APP_NAME))              # Headless mode (generates PNG)")
println("  $(joinpath(OUTPUT_DIR, APP_NAME)) --onscreen  # Interactive onscreen mode")
println()
...
println()
println("Features:")
println("  ✓ Headless rendering to PNG")
println("  ✓ Interactive onscreen window via Rust FFI")
println("  ✓ Event handling (mouse, keyboard, resize)")
println("  ✓ Minimal dependencies (only Rust libs)")
